...
static int __wmi_send(struct wil6210_priv *wil, u16 cmdid, u8 mid,
		      void *buf, u16 len, bool force_send, bool allow_retries)
{
	struct {
		struct wil6210_mbox_hdr hdr;
	...
	for (retry = 5; retry > 0; retry--) {
		...
		if (d_head.sync == 0 || !allow_retries)
			break;
		msleep(20);
	}
	if (d_head.sync != 0) {
	...
	for (retry = 5; retry > 0; retry--) {
		...
		if (next_head != r->tail || !allow_retries)
			break;
		msleep(20);
	}
	if (next_head == r->tail) {
...
static void wmi_evt_ready(struct wil6210_vif *vif, int id, void *d, int len)
{
	...
	wil_info(wil, "FW ver. %s(SW %d); MAC %s; %d MID's\n",
		 wil->fw_version, le32_to_cpu(evt->sw_version),
		 mac_to_str(evt->mac, macstr), evt->numof_additional_mids);
	...
	strlcpy(wiphy->fw_version, (char *)wil->fw_version, sizeof(wiphy->fw_version));
...
#else
	ASSERT(!test_bit(wil_vif_ft_roam, vif->status));
#endif
			      ...
			      void *d, int len)
{
	...
	wil_dbg_wmi(wil, "BACK[%d] %s {%d} timeout %d AMSDU%s\n",
		    ...
		    evt->agg_wsize, le16_to_cpu(evt->ba_timeout),
		    evt->amsdu ? "+" : "-");
...
__acquires(&sta->tid_rx_lock) __releases(&sta->tid_rx_lock)
{
	...
	u8 cid, tid;
	u16 reason = le16_to_cpu(evt->reason);
#ifndef WIL6210_PMD
	struct wil_sta_info *sta;
...
static void wmi_call_async_completion(struct wil6210_priv *wil);

static void wmi_complete(struct wil6210_priv *wil)
{
	if (wil->wmi_async_pending) {
		wmi_call_async_completion(wil);
	} else {
...
static int
wmi_handle_async_event(struct wil6210_priv *wil, u16 reply_id, void *reply_buf)
{
	struct {
		struct wmi_cmd_hdr wmi;
		struct wmi_internal_fw_ioctl_event evt;
	} __packed *reply = reply_buf;
	...
	case WMI_INTERNAL_FW_IOCTL_EVENTID:
		...
		/* response may be ignored, so use debug level for these errors */
		if (wil->ioctl_resp_buf == NULL || wil->ioctl_resp_len == NULL) {
			wil_dbg_wmi(
				wil,
				"ioctl response buffer pointer or response length pointer is null\n");
			rc = -EINVAL;
			break;
		}
		if (evt_len > *wil->ioctl_resp_len) {
			wil_dbg_wmi(
				wil,
				"ioctl response buffer too short (have %d need %d)\n",
				*wil->ioctl_resp_len, evt_len);
			rc = -EINVAL;
			break;
		}
		memcpy(wil->ioctl_resp_buf, &reply->evt.payload, evt_len);
		*wil->ioctl_resp_len = evt_len;
		break;
	default:
		break;
...
int
wmi_call_async(struct wil6210_priv *wil, u16 cmdid, u8 mid, void *buf, u16 len,
	       u16 reply_id, void *reply, u16 reply_size, u64 to_msec)
{
	...
	wil->cmdid = cmdid;
	wil->wmi_async_pending = true;

	rc = __wmi_send(wil, cmdid, mid, buf, len, false, false);
	if (rc)
		goto out;

	wil->wmi_async_sent_time_ms = curr_time_ms;
	wil->wmi_to_msec = to_msec;

	return rc;
out:
	wil->wmi_async_pending = false;
	wil->cmdid = 0;
...
/*
 * This will be called in either the eal-intr-thread or the wmi_event worker
 * thread, depending on the type of wmi call and whether not a reply buffer is
 * allocated for it (immed_reply in wmi_recv_cmd).
 * Do not call this from a data path thread.
 */
static void wmi_call_async_completion(struct wil6210_priv *wil)
{
	ulong flags;
	...
	wil_dbg_wmi(wil,
		    "wmi_call_async(0x%04x->0x%04x) completed in %ld msec\n",
		    wil->cmdid, reply_id, curr_time_ms - wil->wmi_async_sent_time_ms);

	wmi_handle_async_event(wil, reply_id, reply);

	wil->wmi_async_pending = false;
	wil->cmdid = 0;
	wil->wmi_async_sent_time_ms = 0;
	wil->wmi_to_msec = 0;
...
void wmi_call_async_timeout(struct wil6210_priv *wil)
{
	...
	wil_err(wil, "wmi_call_async(0x%04x->0x%04x) timeout after %ld msec\n",
		wil->cmdid, reply_id, curr_time_ms - wil->wmi_async_sent_time_ms);
...
int wmi_set_non_commercial_use(struct wil6210_priv *wil)
{
	...
	return wmi_send(wil, WMI_ALLOW_NON_COMMERCIAL_USE_CMDID, vif->mid,
			NULL, 0);
}
			     ...
			     struct wil6210_mbox_hdr *hdr)
{
	...
	if ((hdr->type == WIL_MBOX_HDR_TYPE_WMI) &&
		...
		print_hex_dump_level(RTE_LOG_ERR, "evt?? ", DUMP_PREFIX_OFFSET, 16, 1,
