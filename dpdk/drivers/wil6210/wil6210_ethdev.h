...
#define DEBUG_LATENCY 1  /* Collect stats for latency testing */
//#define ENABLE_PMC_STATS 1  /* Collect performance monitoring stats */
#ifdef DEBUG_LATENCY
...
#error please enable DEBUG_LATENCY for PMC stats
#endif
#endif

#define XSTATS_FLAGS_PMC_COUNTERS_3 0x1
#define XSTATS_FLAGS_PMC_COUNTERS_6 0x2
/* Make pmc counters clear on read */
#define XSTATS_FLAGS_PMC_COUNTERS_CLEAR 0x4
/* Make counters clear on read */
#define XSTATS_FLAGS_COUNTERS_CLEAR 0x8
...
#define WIL_FW_FILE_PATH_BUFSZ 128
...
#define WIL6210_MAX_TX_RINGS	(16) /* HW limit */
...
struct wil_txrx_ops {
	...
	void (*get_reorder_params)(struct wil6210_priv *wil,
				   struct rte_mbuf *skb, int *tid, int *cid,
				   int *mid, u16 *seq, int *mcast, int *retry);
	void (*get_netif_rx_params)(struct rte_mbuf *skb,
				    int *cid, int *security);
	int (*rx_crypto_check)(struct wil6210_priv *wil, struct rte_mbuf *skb);
	int (*rx_error_check)(struct wil6210_priv *wil, struct rte_mbuf *skb,
...
struct wil6210_priv {
	...
	/* for async wmi calls */
	struct mutex wmi_async_mutex;
	bool wmi_async_pending;
	u64 wmi_async_sent_time_ms;
	u64 wmi_to_msec;
	u16 cmdid;
	u8 *ioctl_resp_buf;
	u16 *ioctl_resp_len;
	...
	/* Controls what/how extended stats are printed */
	u32 xstats_flags;
#ifdef DEBUG_LATENCY
	/* latency stats */
	u64 last_burst_rx_nano;
	u64 max_rx_diff_nano;
	u64 wil_rx_handler_time_nano;

	u64 cycle_per_sec; // store the number of cycle per second, of RTE timer lib.
	u64 nano_per_cycle; // number of nanosec in one RTE timer cycle
#endif

#ifdef ENABLE_PMC_STATS
	u64 wil_tx_handler_time_nano;
#define	PMC_NUM_COUNTER_INSTANCES 8
#define PMC_COUNTERS_RX_INNER1 0
#define PMC_COUNTERS_RX_INNER2 1
#define PMC_COUNTERS_RX_INNER3 2
#define PMC_COUNTERS_RX_OUTER 3
#define PMC_COUNTERS_TX_INNER1 4
#define PMC_COUNTERS_TX_INNER2 5
#define PMC_COUNTERS_TX_INNER3 6
#define PMC_COUNTERS_TX_OUTER 7
	u64 pmc[PMC_NUM_COUNTER_INSTANCES][RTE_PMC_NUM_COUNTERS];
#endif

	char fw_core_dump_path[WIL_FW_FILE_PATH_BUFSZ];
	char fw_str_path[WIL_FW_FILE_PATH_BUFSZ];
	char fw_log_path[WIL_FW_FILE_PATH_BUFSZ];
	char ucode_str_path[WIL_FW_FILE_PATH_BUFSZ];
	...
	size_t ucode_log_offset;
	size_t ucode_log_buf_entries;
	struct wil_fw_log_state *fw_log_state;
	struct wil_fw_log_state *ucode_log_state;
	pthread_t log_poll_thread;
	int log_pipe[2];
};
...
#define wil_err(wil, arg...)  dev_printf(ERR, wil_to_dev(wil), arg)
#define wil_info(wil, arg...) dev_printf(INFO, wil_to_dev(wil), arg)
#define wil_err_ratelimited(wil, arg...) \
	dev_printf(ERR, wil_to_dev(wil), arg)
#define wil_dbg_ratelimited(wil, arg...) \
	dev_printf(DEBUG, wil_to_dev(wil), arg)

/*
 * Simulate wil_bgd dynamic prints using dynamic log types in DPDK
 */
enum wil_dbg_type {
	WIL_DBG_LOG_IRQ,
	WIL_DBG_LOG_TXRX,
...
#define WIL_DBG_LOG_MASK_IRQ	(1 << WIL_DBG_LOG_IRQ)
#define WIL_DBG_LOG_MASK_TXRX	(1 << WIL_DBG_LOG_TXRX)
#define WIL_DBG_LOG_MASK_WMI	(1 << WIL_DBG_LOG_WMI)
#define WIL_DBG_LOG_MASK_MISC	(1 << WIL_DBG_LOG_MISC)
...
#define wil_dbg(wil, type, fmt, arg...) do { \
	if (unlikely(wil->dbg_log_enable_mask & WIL_DBG_LOG_MASK_##type)) \
		wil_dbg_trace(wil, WIL_DBG_LOG_##type, "%s: " fmt, \
		    wil_to_dev(wil)->name, ##arg); \
} while (0)

#define wil_dbg_irq(wil, fmt, arg...) wil_dbg(wil, IRQ, "DBG[ IRQ]" fmt, ##arg)
#define wil_dbg_txrx(wil, fmt, arg...) wil_dbg(wil, TXRX, "DBG[TXRX]" fmt, ##arg)
#define wil_dbg_wmi(wil, fmt, arg...) wil_dbg(wil, WMI, "DBG[ WMI]" fmt, ##arg)
...
#define WIL_RTE_LOG_LEVEL  RTE_LOG_LEVEL
#else
#define WIL_RTE_LOG_LEVEL  RTE_LOG_DP_LEVEL
#endif

#if WIL_RTE_LOG_LEVEL >= RTE_LOG_DEBUG
#define wil_hex_dump_txrx(prefix_str, prefix_type, rowsize,	\
			  groupsize, buf, len, ascii)		\
			  print_hex_dump_debug(WIL_DBG_LOG_TXRX,\
					 "DBG[TXRX]" prefix_str,\
					 prefix_type, rowsize,	\
					 groupsize, buf, len, ascii)
...
#define wil_hex_dump_wmi(prefix_str, prefix_type, rowsize,	\
			 groupsize, buf, len, ascii)		\
			 print_hex_dump_debug(WIL_DBG_LOG_WMI,  \
					"DBG[ WMI]" prefix_str, \
					prefix_type, rowsize,	\
					groupsize, buf, len, ascii)
...
#define wil_hex_dump_misc(prefix_str, prefix_type, rowsize,	\
			  groupsize, buf, len, ascii)		\
			  print_hex_dump_debug(WIL_DBG_LOG_MISC,\
					"DBG[MISC]" prefix_str, \
					prefix_type, rowsize,	\
					groupsize, buf, len, ascii)
...
#define wil_hex_dump_umac(prefix_str, prefix_type, rowsize,	\
			  groupsize, buf, len, ascii)		\
			  print_hex_dump_debug(WIL_DBG_LOG_UMAC,\
					"DBG[UMAC]" prefix_str, \
					prefix_type, rowsize,	\
					groupsize, buf, len, ascii)
...
int wmi_set_pcie_config_params(struct wil6210_priv *wil, u32 pcie_gen,
			       u32 pcie_lane_count);
...
void wil_delete_peers(struct wil6210_priv *wil);
...
netdev_tx_t wil_start_xmit(struct rte_mbuf *skb, struct net_device *ndev);
netdev_tx_t _wil_start_xmit(struct rte_mbuf *skb, struct net_device *ndev);
...
void wil_fw_not_ready_logs(struct wil6210_priv *wil);
void wil_fw_set_log_offset_entries(struct wil6210_priv *wil);
void wil_fw_set_log_level(struct wil6210_priv *wil);
...
int wil6210_dev_uninit(struct rte_eth_dev *eth_dev);

int wil6210_eth_dev_init(struct rte_eth_dev *dev);
int wil6210_eth_dev_stop(struct rte_eth_dev *dev);

uint16_t wil_rx_burst(struct wil6210_priv *wil, struct rte_mbuf **rx_pkts,
    int16_t nb_pkts);

void dhd_rx_setup(void *ctx, uint16_t in_port, struct rte_mempool *mb_pool);
uint16_t dhd_rx_burst(void *qdata, struct rte_mbuf **bufs, uint16_t nb_pkts);
uint16_t dhd_tx_burst(void *qdata, struct rte_mbuf **bufs, uint16_t nb_pkts);

void wil_eth_tx_ring_enable(struct wil6210_priv *wil, unsigned int cid,
			    unsigned int qid);
void wil_eth_tx_ring_disable(struct wil6210_priv *wil, unsigned int cid,
			     unsigned int qid);

void wil_api_link_up(struct wil6210_priv *wil, unsigned int qid,
		     u8 addr[ETH_ALEN]);
...
int wil_send_queue_stats_ioctl_async(struct wil6210_priv *wil,
				     ...
				     struct wil_sta_info **stas);
uint16_t wil_tx_ring_config_size(void);
/*
 * Global map of all TG links. Should remain constant once PMD driver
 * has started and attached to all devices. It might be better to convert
 * this to per-port data in the future.
 */
#include "dpdk-dhd-ctrl.h"

struct dhd_link_info {
	struct dhd_state *dhd;
	int dhd_port_id;
	int dhd_peer_id;
	int dhd_peer_nameunit;
	int dhd_peer_ifindex;
	char dhd_peer_ifname[IFNAMSIZ];
};

extern struct dhd_link_info dhd_link_map[DHD_TOTAL_LINKS];
